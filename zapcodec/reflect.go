...
// reflectiveCodec is the reflection-driven (un)marshaller. It mirrors
// reflectcodec.genericCodec's structure but plugs into the zapcodec
// little-endian packer.
//
// Why a sibling impl instead of reusing reflectcodec? reflectcodec is
// tied to wrappers.Packer (big-endian). The whole point of zapcodec is
// the little-endian wire format — we can't share the marshal/unmarshal
// body without a generics-shaped indirection that would defeat the
// per-field speedup. We DO share reflectcodec.StructFielder, because
// that's pure type-shape inspection with no encoding involved.
type reflectiveCodec struct {
	typer   typeCoder
	fielder reflectcodec.StructFielder
}

// typeCoder is the interface/registry-driven prefix codec. The
// zapcodec-local linearCodec implements this. Shape mirrors
// reflectcodec.TypeCodec.
...
func newReflective(t typeCoder, tagNames []string) *reflectiveCodec {
	return &reflectiveCodec{
		typer:   t,
		fielder: reflectcodec.NewStructFielder(tagNames),
	}
}
...
func (c *reflectiveCodec) marshal(value reflect.Value, p *packer, stack set.Set[reflect.Type]) error {
	...
	case reflect.Slice:
		...
		if n > math.MaxInt32 {
			return fmt.Errorf("%w; slice length %d exceeds %d",
				codec.ErrMaxSliceLenExceeded, n, math.MaxInt32)
		}
		p.PackInt(uint32(n))
		if p.err != nil {
			return p.err
	...
	case reflect.Map:
		...
		return nil
	default:
		return fmt.Errorf("%w: %s", codec.ErrUnsupportedType, kind)
	}
}
	...
	case reflect.Slice:
		...
		if n32 > math.MaxInt32 {
			return fmt.Errorf("%w; array length %d exceeds %d",
				codec.ErrMaxSliceLenExceeded, n32, math.MaxInt32)
		}
		n := int(n32)
		sliceT := value.Type()
		innerT := sliceT.Elem()
