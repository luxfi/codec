...
func newManager(t *testing.T) codec.Manager {
	t.Helper()
...
// multi-byte integers. The codec.Manager's own 2-byte version prefix
// is BigEndian (per codec.Manager.Marshal); everything past byte 2 is
// the codec's output and must be LE.
...
func TestMarshalNil(t *testing.T) {
	...
	require.ErrorIs(t, err, codec.ErrMarshalNil)
}
...
// TestDuplicateRegistration asserts the codec.Registry contract — the
// same type registered twice returns ErrDuplicateType.
func TestDuplicateRegistration(t *testing.T) {
	c := NewDefault()
	require.NoError(t, c.RegisterType(&circle{}))
	err := c.RegisterType(&circle{})
	require.ErrorIs(t, err, codec.ErrDuplicateType)
}
